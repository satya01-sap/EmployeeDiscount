...
entity Cards {
  key ID: UUID;
  cardNumber: CreditCardType; //String(25) @assert.unique @mandatory;
  cardName: String(99);
  cardType: String(2);
  issueDate: Date;
  expiryDate: Date;
...
type CreditCardType : String(12) 
  @mandatory
  @assert.unique
  @assert.format: '^\d{12}$'
  @assert.format.message: 'Credit card must be exactly 12 digits long and contain numbers only.';
